...
function calculateRating(watchList) {
    var list = watchList.filter(function (movie) {
        return movie.Director === 'Christopher Nolan';
    });
    var sumRating = list.reduce(function (total, rating) {
        return total + parseFloat(rating.imdbRating);
    }, 0);
    var IMDB = sumRating / list.length;
    return IMDB;
}
// Expected results
...
var sum = numbers.myreduce((total, number) => {
    return total + number;
}, 10);
console.log(sum);
console.log('==================');
// dùng reduce nối object
function arrToObj(arr) {
    var name = arr.reduce(function (el, item) {
        el[item[0]] = item[1];
        return el;
    }, {});
    return name;
